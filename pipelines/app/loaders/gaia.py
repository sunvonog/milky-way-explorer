...
def load(snapshot: Path) -> pl.DataFrame:
    ...
    batch_paths = sorted(path for path in batches_root.glob("gaia-host-*.csv") if path.is_file())

    if not batch_paths:
        raise FileNotFoundError(f"no Gaia batch files found in: {batches_root}")
