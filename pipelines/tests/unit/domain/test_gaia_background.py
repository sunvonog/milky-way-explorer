...
def test_builds_density_ready_background_sources():
    staging = pl.DataFrame(
        {
            ...
            "gaia_source_id": [3, 1, 4, 2],
            "galactic_longitude_deg": [0.0, 0.0, 0.0, 90.0],
            "galactic_latitude_deg": [0.0, 0.0, 0.0, 0.0],
            "distance_gspphot_pc": [None, 25.0, 30.0, None],
            "parallax_mas": [10.0, 10.0, 10.0, 10.0],
            "parallax_over_error": [2.5, 10.0, 10.0, 10.0],
            "ruwe": [1.0, 1.0, 1.0, 1.0],
            "phot_g_mean_magnitude": [13.0, 10.0, 9.0, 11.0],
            "bp_rp_color": [1.3, 0.5, 0.2, 0.8],
            "is_valid": [True, True, False, True],
            "source": ["gaia_background"] * 4,
        },
    ...
    assert actual["gaia_source_id"].to_list() == [1, 2, 3]

    assert actual.select(
        "gaia_source_id", "distance_pc", "distance_method", "distance_quality"
    ).to_dicts() == [
        {
            "gaia_source_id": 1,
            "distance_pc": 25.0,
    ...
    assert actual["phot_g_mean_magnitude"].to_list() == [10.0, 11.0, 13.0]
