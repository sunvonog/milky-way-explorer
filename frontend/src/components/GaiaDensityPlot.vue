...
import { buildGaiaDensityLayer } from '@/visualization/gaiaDensityLayer'
...
const mapLayers = computed(() => {
  const gridLevel = selectedGridLevel.value
  ...
  <figure v-else class="m-0 grid gap-3 text-slate-100">
    ...
    <details class="rounded-xl border border-slate-800 p-3">
      ...
      >
        ...
        >
          <title>
            {{ cell.record.sourceCount }} Gaia sources - {{ cell.record.distanceTier }} distance
            tier
          </title>
        </rect>
