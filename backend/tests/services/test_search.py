...
def test_search_raises_when_unpublished(tmp_path: Path) -> None:
    with pytest.raises(BuildNotPublishedError):
        search(tmp_path / "processed", "Sirius")


def test_search_exact_match(search_dataset: Path) -> None:
...
def test_search_prefix_ranking(search_dataset: Path) -> None:
...
def test_search_empty_key_returns_empty(search_dataset: Path) -> None:
